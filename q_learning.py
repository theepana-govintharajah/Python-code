...
def q_learning_control_epsilon_greedy(
    ...
    """
    Q-learning (off-policy TD control) with epsilon-greedy BEHAVIOR policy.

      Initialize S
      Repeat:
        Choose A ~ b(.|S) (epsilon-greedy from Q)          [behavior: epsilon-soft, random tie-break]
        Take A, observe R, S'
        Q(S,A) <- Q(S,A) + alpha * [ R + gamma*max_a' Q(S',a') - Q(S,A) ]   [target: greedy]
        S <- S'
      until terminal

    Key difference vs SARSA:
      - SARSA uses Q(S',A') where A' ~ behavior policy (on-policy)
      - Q-learning uses max_a' Q(S',a') regardless of behavior (off-policy)
    """
    ...
    for ep in range(1, num_episodes + 1):
        ...
        for _ in range(max_steps_per_episode):
            # Choose action from S using epsilon-soft behavior policy (RANDOM tie-break for greedy)
            ...
            # Q-learning target uses GREEDY action value at next state (off-policy)
            best_a_ns = _argmax_fixed_tie(Q[ns])  # deterministic max for target
            td_target = r + gamma * Q[ns][best_a_ns]
            Q[s][a] += alpha * (td_target - Q[s][a])
        ...
        if verbose_every is not None and ep % int(verbose_every) == 0:
            ...
            print("[Q-Learning] episode={} | eps={:.4f} | greedy success_rate={:.3f}".format(ep, eps_t, sr))
