...
if __name__ == "__main__":
    # Task 1 default: 4x4 with 4 holes (25%)
    holes_4x4 = {(1, 1), (1, 3), (2, 3), (3, 0)}  # example; you can change
    env = FrozenLakeEnv(n=4, holes=holes_4x4, seed=40)
    s = env.reset()
    env.render()
    for _ in range(10):
        a = env.sample_action()
        ns, r, done, _ = env.step(a)
        print("a={}, ns={}, r={}, done={}".format(FrozenLakeEnv.ACTION_NAMES[a], ns, r, done))
        env.render()
        if done:
            break

    # Task 2: random 10x10 with 25% holes, guaranteed solvable
    holes_10x10 = generate_random_solvable_holes(n=10, hole_ratio=0.25, seed=40)
    env2 = FrozenLakeEnv(n=10, holes=holes_10x10, seed=40)
    env2.reset()
    env2.render()
    for _ in range(10):
        a = env2.sample_action()
